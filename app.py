...
def get_sheet_data_fresh(mode_key):
    ...
    try:
        ...
        try: ws = sh.worksheet(target_sheet_name)
        except: 
            ws = sh.add_worksheet(title=target_sheet_name, rows=1000, cols=20)
...
def api_call_vision_ocr(image_bytes):
    prompt_text = "이 이미지에 있는 텍스트를 보이는 그대로 추출해주세요. 말풍선, 단락은 줄바꿈으로 구분하세요."
