...
# 페이지 기본 설정
st.set_page_config(
    page_title="국어활동 AI 분석기 (Debug Ver.)", 
    page_icon="📚", 
    layout="wide",
...
st.markdown("""
    <style>
        ...
        /* 디버그 메시지 스타일 */
        .debug-msg { color: #ffa500; font-size: 0.85rem; font-family: monospace; }
    </style>
...
def api_call_direct(prompt, image_bytes=None):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if st.session_state.get('debug_mode'): st.error(f"API Error: {res.text}")
            return None
    except Exception as e:
        if st.session_state.get('debug_mode'): st.error(f"API Connection Error: {e}")
        return None
...
def api_call_vision_ocr(image_bytes):
    ...
    data = {"contents": [{"parts": [{"text": "텍스트 추출(줄바꿈 준수, 원본 유지)", "inline_data": {"mime_type": "image/png", "data": b64}}]}]}
    try:
        res = requests.post(url, headers=headers, json=data, timeout=300)
        if res.status_code == 200: return res.json()['candidates'][0]['content']['parts'][0]['text']
        return ""
    except Exception as e:
        if st.session_state.get('debug_mode'): st.error(f"OCR Error: {e}")
        return ""
...
# =========================================================
# [4] 파일 처리 (디버깅 모드 + BytesIO 완전 격리)
# =========================================================
def extract_text_unified(file_bytes, file_type, page_idx):
    """
    파일 데이터를 bytes로 직접 받아 처리 (파일 포인터 문제 원천 차단)
    """
    debug = st.session_state.get('debug_mode', False)
    if debug: st.caption(f"🔍 [Debug] 텍스트 추출 시도: {file_type} (Page {page_idx})")

    if "image" in file_type:
        if debug: st.caption("👉 이미지 파일 -> Vision OCR 호출")
        ...
        if PLUMBER_AVAILABLE:
            try:
                if debug: st.caption("👉 1차 시도: PDFPlumber")
                ...
                if debug: st.caption(f"❌ Plumber Error: {e}")

        # 2. Fitz
        if (not text or len(text.strip()) < 5) and FITZ_AVAILABLE:
            try:
                if debug: st.caption("👉 2차 시도: PyMuPDF (Fitz)")
                doc = fitz.open(stream=file_bytes, filetype="pdf")
                if page_idx < len(doc): text = doc[page_idx].get_text()
            except Exception as e:
                if debug: st.caption(f"❌ Fitz Error: {e}")
        
        # 3. Vision OCR
        if (not text or len(text.strip()) < 30) and FITZ_AVAILABLE:
            try:
                if debug: st.caption("👉 3차 시도: Vision OCR (이미지 변환)")
                doc = fitz.open(stream=file_bytes, filetype="pdf")
                if page_idx < len(doc):
                    pix = doc[page_idx].get_pixmap(matrix=fitz.Matrix(2, 2))
                    return api_call_vision_ocr(pix.tobytes("png"))
            except Exception as e:
                if debug: st.caption(f"❌ Vision Fallback Error: {e}")
                
        if debug: st.caption(f"✅ 추출 완료: {len(text) if text else 0}자")
...
if 'file_bytes_cache' not in st.session_state: st.session_state.file_bytes_cache = None # 파일 데이터 캐시
...
if main_file:
    # [핵심] 파일 데이터 자체를 해싱하여 고유 ID 생성 (AttributeError 방지)
    # 파일을 딱 한 번만 읽어서 세션에 '복제'해둠 (빈 텍스트 방지)
    current_bytes = main_file.getvalue()
    file_hash = hashlib.md5(current_bytes).hexdigest()
    
    if st.session_state.file_hash != file_hash:
        st.session_state.file_hash = file_hash
        st.session_state.file_bytes_cache = current_bytes # 데이터 영구 보존
