...
def get_analysis_hybrid(text, image_bytes, sheet_data, mode_key):
    ...
    1. **한 (한자어)**: 한자(Chinese Characters)에 기반한 단어는 반드시 '한'으로 분류하세요.
       - 예시: 학교(學校), 분석(分析), 도서관(圖書館), 감탄(感歎), 질문(質問), 입력(入力)
       - 팁: 의미가 명확하고 격식 있는 명사는 한자어일 확률이 매우 높습니다.
    2. **고 (고유어)**: 순우리말.
       - 예시: 하늘, 바다, 가다, 아름답다, 사람, 나무
    3. **외 (외래어)**: 영어 등 서양 언어에서 유래한 단어.
       - 예시: 버스, 컴퓨터, 커피, 데이터
    4. **혼 (혼종어)**: 서로 다른 어종이 결합된 단어.
    
    [출력 포맷]
    반드시 아래 **한글 키(Key)**를 가진 JSON 리스트로 출력하세요.
    - 원본: 문장에서 쓰인 단어 (조사/어미 제외한 형태 권장)
    - 원형: 기본형 (사전 등재형)
    - 분류: 어종 (고/한/외/혼) - 위 분류 기준을 엄수할 것
