...
def save_logic_common():
    ...
    for _, row in res_df.iterrows():
        if not row.get('삭제', False):
            logs.append([datetime.now().isoformat(), row.get('원본',''), row.get('원형',''), clean_val_for_save(row.get('분류','')), clean_val_for_save(row.get('품사','')), 'modify', 'result'])
    if logs: send_data_with_retry(sheet, logs, True)
...
# =========================================================
# [4] AI 분석 엔진 (동음이의어 및 인명/지명 분류 고도화)
# =========================================================
def generate_prompt_from_sheet(sheet_data):
    if not sheet_data: return ""
    rules = []
    for row in sheet_data[-100:]:
        orig, root, origin, pos = row.get('original_word', ''), row.get('root_word', ''), row.get('origin', ''), row.get('pos', '')
        if row.get('action') == 'delete': rules.append(f"- [제외]: '{orig}'는 분석에서 뺍니다.")
        elif row.get('action') in ['add', 'modify']: rules.append(f"- [고정]: '{orig}' -> 원형:'{root}', 분류:'{origin}', 품사:'{pos}'")
    return "\n[사용자 학습 데이터]:\n" + "\n".join(rules) + "\n"
...
def extract_text_unified(file_bytes, file_type, page_idx):
    raw_text = ""
    if "image" in file_type: raw_text = api_call_direct("텍스트를 모두 추출해줘.", file_bytes) or ""
...
def get_analysis_hybrid(text, image_bytes, sheet_data, mode_key):
    # '명사+하다' 규칙 삭제 & 동음이의어/인명/지명 규칙 강화
    prompt = f"""
    당신은 국어 형태소 분석 전문가입니다. 아래 규칙에 따라 텍스트를 정밀 분석하여 JSON 리스트로 출력하세요.
    {generate_prompt_from_sheet(sheet_data)}
    
    [분석 핵심 규칙]
    1. **특수문자 및 문장부호 제거**: 마침표, 쉼표, 괄호 등 모든 특수문자는 결과에 절대 포함하지 마세요.
    2. **조사 및 어미 제외**: 조사(~은/는/이/가)와 어미(~다/요)는 독립된 항목으로 출력하지 마세요.
    3. **의존 명사**: '것', '수', '만큼' 등은 '명사'로 분류하여 포함하세요.
    4. **동음이의어 정밀 처리**: 문맥상 뜻이 갈리는 단어는 반드시 원형 뒤에 괄호로 뜻을 구분하세요 (예: 배(과일), 배(선박)).
    5. **인명 및 지명 자동 분류**: 문맥상 사람이름은 원형 뒤에 '(이름)', 지역이름은 '(지명)'을 붙여 원형을 추출하세요 (예: 지혜(이름), 서울(지명)).
    ...
    [출력 포맷: 반드시 아래 한글 키를 가진 JSON 리스트]
...
# STEP 2: 자료 입력 (상태 유지 기능 포함)
    ...
    def run_analysis(txt, img=None):
        ...
        with st.spinner("AI 분석 중..."):
            ...
            for r in res:
                o = str(r.get('원본') or r.get('original_word') or '').strip()
                root = str(r.get('원형') or r.get('root_word') or '').strip()
                if not o or not root or not re.search(r'[가-힣a-zA-Z0-9]', o): continue
                if str(r.get('품사')) in ['조사', '어미', '문장부호']: continue
                key = (root, str(r.get('분류','혼')), str(r.get('품사','명사')))
                if key not in temp: temp[key] = []
                temp[key].append(o)
            for (root, origin, pos), origs in temp.items():
                cnts = Counter(origs)
...
# STEP 3: 결과 확인 (연속 작업 흐름 최적화)
    ...
    edited = st.data_editor(df, column_config={"삭제": st.column_config.CheckboxColumn("삭제"), "원본": st.column_config.TextColumn("원본", disabled=True), "분류": st.column_config.SelectboxColumn("분류", options=["🔵 고", "🟢 한", "🔴 외", "🟣 혼"]), "품사": st.column_config.SelectboxColumn("품사", options=["📦 명사", "🏃 동사", "🎨 형용사", "⚡ 부사", "🔍 관형사", "👤 대명사", "고유명사", "❗ 감탄사"])}, use_container_width=True, num_rows="dynamic", key="editor")
    if not edited.equals(df): st.session_state.analysis_result = edited.to_dict('records')
    ...
    if not st.session_state.is_finished:
        ...
        with b2:
            if st.button("⛔ 선택 삭제", use_container_width=True): st.session_state.analysis_result = edited[edited['삭제']==False].to_dict('records'); st.rerun()
        with b3:
            if st.button("💾 저장하기 (완료)", type="primary", use_container_width=True): save_logic_common(); st.session_state.is_finished = True; st.balloons(); st.rerun()
    else:
        st.success("✅ 모든 분석 데이터가 저장되었습니다!")
