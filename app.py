...
def extract_text_unified(file_bytes, file_type, page_idx):
    raw_text = ""
    if "image" in file_type: raw_text = api_call_direct("이 이미지의 텍스트를 보이는 그대로 추출해줘. 줄바꿈 유지.", file_bytes) or ""
        ...
        if (not raw_text or len(raw_text) < 10) and FITZ_AVAILABLE:
             try:
                ...
                if page_idx < len(doc):
                    ...
                    raw_text = api_call_direct("이 이미지의 텍스트를 보이는 그대로 추출해줘. 줄바꿈 유지.", pix.tobytes("png")) or ""
             except: pass
...
def get_analysis_hybrid(text, image_bytes, sheet_data, mode_key):
    ...
    단순히 한글로 표기되었다고 '고(고유어)'로 판단하지 말고, 한자어 여부를 엄격히 판단하세요.
    1. **한 (한자어)**: 한자(Chinese Characters)에 기반한 단어는 반드시 '한'으로 분류하세요.
       - 예시: 학교(學校), 분석(分析), 질문(質問), 입력(入力), 감탄(感歎)
    2. **고 (고유어)**: 순우리말. 예: 하늘, 바다, 가다, 아름답다
...
# ---------------------------------------------------------
# STEP 0: 시작 화면 (오션 블루 / 로즈 레드 상징색 적용)
# ---------------------------------------------------------
if st.session_state.step == 0:
    ...
    with col1:
        with st.container(border=True):
            ...
            if up_excel:
                ...
                except: st.error("파일 형식이 잘못되었습니다.")
    with col2:
        with st.container(border=True):
...
# ---------------------------------------------------------
    ...
    input_method = st.radio("입력 방식", ["📄 파일 분석", "✍️ 직접 입력"], horizontal=True, index=st.session_state.current_tab_idx, label_visibility="collapsed")

    # 분석 실행 공통 로직
    def run_analysis_action(txt, img=None):
        if not txt or not txt.strip(): st.warning("⚠️ 분석할 텍스트가 없습니다."); return
        with st.spinner("AI 분석 중... (어종 분류 정밀 분석 포함)"):
    ...
    if input_method == "📄 파일 분석":
        ...
        if file:
            ...
            with c2:
                txt_in = st.text_area("에디터 (수정 가능)", value=st.session_state.extracted_text, height=500)
...
# ---------------------------------------------------------
# STEP 3: 결과 확인 (수정 페이지로 돌아가기 버튼 포함)
# ---------------------------------------------------------
    ...
    with st.expander("📝 분석된 원문 텍스트 확인"):
    ...
    with a3:
        if st.button("💾 저장하기 (완료)", type="primary", use_container_width=True):
            save_logic_common(); st.session_state.step = 4; st.rerun()

# ---------------------------------------------------------
# STEP 4: 최종 완료 및 다운로드
# ---------------------------------------------------------
elif st.session_state.step == 4:
    st.balloons(); st.header("✅ 모든 분석이 완료되었습니다!")
    if st.session_state.master_df is not None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.download_button("📥 결과 파일 다운로드", buf.getvalue(), fname, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="primary")
        with c2:
