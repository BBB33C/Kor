...
st.set_page_config(
    page_title="국어활동 AI 분석기 (Final Integrity)", 
    page_icon="📚", 
    layout="wide",
...
def api_call_vision_ocr(image_bytes):
    ...
    prompt_text = "이미지의 텍스트를 보이는 그대로 추출하세요. 두음법칙 미적용, 세로쓰기 등 원본 형태 유지. 줄바꿈 준수."
...
def get_analysis_hybrid(text, image_bytes, sheet_data, mode_key):
    ...
    3. **'하다' 용언 처리 (중요)**: '명사+하다' -> 핵심 명사 추출 (공부하다 -> 공부).
    4. **품사 필터링**: 명사, 동사, 형용사, 부사, 관형사, 대명사만 포함.
    5. **출력**: JSON 포맷.
            ...
            try:
                with pdfplumber.open(file_obj) as pdf:
                    if page_idx < len(pdf.pages):
                        ...
                        try:
                            crop_box = (0, height * 0.05, width, height * 0.9) # GP9 크롭
...
def check_trust_level_strict(root_word, uploaded_df, problematic_words):
    ...
    try: return match.iloc[0]['출연횟수'] >= TRUST_THRESHOLD
    except: return False
...
with st.sidebar:
    ...
    if up_excel and up_excel.name != st.session_state.last_uploaded_file_name:
        if st.button("병합하기"):
            try:
                loaded = load_excel_safely(up_excel)
    ...
    if st.button("🚀 분석 실행", type="primary", use_container_width=True):
        if not txt_val.strip(): st.warning("내용 없음")
            ...
            with st.spinner("분석 중..."):
                ...
                problematic = get_problematic_words(sheet_data)
                ...
                for r in res:
                    ...
                    if root in blacklist: continue # 블랙리스트 체크
                    
                    if root not in seen:
                        # 신뢰도 체크 (GP9 복구)
                        is_trusted = check_trust_level_strict(root, st.session_state.master_df, problematic)
