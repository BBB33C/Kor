...
# 페이지 기본 설정
st.set_page_config(
    page_title="국어활동 AI 분석기 (UI Fixed)", 
    page_icon="📚", 
    layout="wide",
                ...
                with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
                    if page_idx < len(pdf.pages):
                        ...
                        if text and len(text.strip()) > 30:
                            if debug: log_debug(f"Plumber 성공 ({len(text)}자)", "success")
                            return text
            except: pass
        ...
        if FITZ_AVAILABLE:
            try:
                ...
                if page_idx < len(doc):
                    ...
                    if text and len(text.strip()) > 30:
                        if debug: log_debug(f"Fitz 성공 ({len(text)}자)", "success")
                        return text
            except: pass
...
if main_file:
    ...
    if st.session_state.file_hash != file_hash:
        ...
        # [핵심 해결책] 텍스트를 추출해서 세션 키에 '직접' 할당
        extracted_text = extract_text_unified(current_bytes, main_file.type, 0)
        st.session_state["main_editor_area"] = extracted_text # 키에 직접 값을 꽂음
        
        log_debug(f"파일 로드 및 텍스트 추출 완료 ({len(extracted_text)}자)", "success")
        st.rerun()
...
with col_i:
    st.info("📝 분석 내용 (수정 가능)")
    # [핵심] value= 파라미터 제거하고 key로 상태 관리 일원화
